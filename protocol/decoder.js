...
function decodeAliveResponse(buf) {
    ...
    if (result > 0) {
        throw new DecoderError(
            'EPMD returned an error. Result: "' + result + '"'
        );
    }
...
function decodePortResponse(buf) {
    ...
    if (highestVersion < constants.LOWEST_VERSION) {
        throw new DecoderError(
            'Highest protocol version returned lower than our lowest version'
        );
    }
    if (lowestVersion > constants.HIGHEST_VERSION) {
        throw new DecoderError(
            'Lowest protocol version provided higher than our highest version'
        );
    }
    return {
